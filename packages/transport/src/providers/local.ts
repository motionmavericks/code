...
function stableId(req: ChatRequest, seq: number): string {
  ...
  return `local:${seed}:${seq}`;
}

export const name: ProviderAdapter["name"] = "local";

export async function chat(
  req: ChatRequest,
  usage?: UsageAccounting
): Promise<ChatResponse> {
  const text = `[local:${req.model}] ${req.messages.at(-1)?.content ?? ""}`;
...
export async function chatStream(
  ...
  async function* stream(): AsyncGenerator<StreamChunk, ChatResponse, void> {
    ...
    const text = `[local:${req.model}] ${content}`;
