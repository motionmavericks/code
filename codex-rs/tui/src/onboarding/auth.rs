...
impl AuthModeWidget {
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            Line::from(""),
            Line::from("  Decide how much autonomy you want to grant Code OS"),
            Line::from(vec![
                Span::raw("  For more details see the "),
                Span::styled(
                    "\u{1b}]8;;https://github.com/just-every/code\u{7}Code OS docs\u{1b}]8;;\u{7}",
                    Style::default().add_modifier(Modifier::UNDERLINED),
                ),
            ])
            .style(Style::default().add_modifier(Modifier::DIM)),
            Line::from(""),
            Line::from("  Code OS can make mistakes"),
    ...
    fn render_env_var_missing(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            Line::from(
                "  To use Code OS with the OpenAI API, set OPENAI_API_KEY in your environment",
            )
